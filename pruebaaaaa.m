...
for k=1:N 
    ...
    % c) Auto-sintonización de las ganancias
    K = autoSintonizarGanancias(Error(k));
...
% Función de auto-sintonización de ganancias
function K = autoSintonizarGanancias(Error)
    % Parámetros de auto-sintonización
    ganancia_min = 0.5;    % Ganancia mínima
    ganancia_max = 1;      % Ganancia máxima
    distancia_umbral = 3;   % Distancia umbral donde se usa la ganancia mínima
    
    % Ajustar las ganancias en función del error de posición
    if Error <= distancia_umbral
        K = ganancia_max * eye(2); % Usar la ganancia máxima para errores pequeños
    else
        K = ganancia_min * eye(2);
    end
end